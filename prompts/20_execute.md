...
4. **Run the project's own tests/build if one is discoverable.** Check for an obvious command (`npm test`, `pytest`, `make test`, a CI config, a README section) before you decide there isn't one. Test output is worth a `send_update` even before you're done -- see below. If it's a Python project (a `requirements.txt`/`pyproject.toml`/`setup.py`) with no `.venv` yet at the workspace root, create one before installing anything: `python3 -m venv .venv && .venv/bin/pip install -r requirements.txt` (or the project's equivalent). This only needs doing once -- every `run` command after that automatically activates the workspace root's `.venv` if one exists, so plain `pip`/`python` already resolve inside it. Likewise, if the project already pins a Node version via `.nvmrc`/`.node-version`, every `run` command already switches to it (installing it first if needed) before running -- you don't need to invoke `nvm` yourself. If `npm`/`node` still comes back not found after that, nvm itself isn't set up in this environment -- that's the kind of environment limitation the Bias section below is for, not something to keep retrying.
